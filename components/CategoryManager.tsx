...
export default function CategoryManager({ initialCategories }: { initialCategories: Category[] }) {
  ...
  return <main className="p-5 sm:p-8 lg:p-10">
    ...
    <section className="mt-8 rounded-2xl border border-color4/80 bg-color3 p-5 shadow-sm sm:p-7"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-color4 text-color1"><Tag size={20}/></span><div><h2 className="text-xl font-bold">{editingId ? "Edit Kategori" : "Tambah Kategori"}</h2><p className="text-sm text-color5/60">Masukkan nama kategori usaha.</p></div></div><form onSubmit={submit} className="mt-6 flex flex-col gap-3 sm:flex-row"><input value={name} onChange={(event) => setName(event.target.value)} placeholder="Contoh: Makanan & Minuman" className="h-12 flex-1 rounded-xl border border-color4 bg-color3 px-4 outline-none focus:border-color1 focus:ring-2 focus:ring-color1/15"/><div className="flex gap-3"><button type="button" onClick={reset} className="h-12 rounded-xl border border-color4 px-5 font-bold">Batal</button><button disabled={saving || !name.trim()} className="inline-flex h-12 items-center gap-2 rounded-xl bg-color1 px-5 font-bold text-white disabled:opacity-50">{editingId ? <Edit3 size={18}/> : <Plus size={18}/>}{saving ? "Menyimpan..." : editingId ? "Perbarui" : "Simpan"}</button></div></form>{message && <p className="mt-4 text-sm font-medium text-red-600">{message}</p>}</section>
